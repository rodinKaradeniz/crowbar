...
| `/business/reservations?tab=requests` | Core | RES, SCHED, CRM | Reservations | Pending-request triage. Was its own route `/business/requests` until it became a tab of the reservations workspace; that path now redirects here. Gated on `reservations.manage`, matching the PATCH it performs. |
