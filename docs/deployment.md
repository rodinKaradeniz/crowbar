...
services:
  postgres:
    ...
    container_name: rk-reservations-db
    restart: always
    environment:
      POSTGRES_DB: ${POSTGRES_DB:-rk_reservations}
  ...
  redis:
    ...
    container_name: rk-reservations-redis
    restart: always
    volumes:
      - redis_data:/data
    healthcheck:
  ...
  api:
    ...
    container_name: rk-reservations-api
    restart: always
    ports:
      - "8000:8000"
    environment:
      DATABASE_URL: postgresql+asyncpg://${POSTGRES_USER:-postgres}:${POSTGRES_PASSWORD}@postgres:5432/${POSTGRES_DB:-rk_reservations}
  ...
  ml:
    build:
      context: ../ml
      dockerfile: Dockerfile
    container_name: rk-reservations-ml
    restart: always
    ports:
      - "8001:8001"
    environment:
      DATABASE_URL: postgresql+asyncpg://${POSTGRES_USER:-postgres}:${POSTGRES_PASSWORD}@postgres:5432/${POSTGRES_DB:-rk_reservations}
      DATABASE_URL_SYNC: postgresql://${POSTGRES_USER:-postgres}:${POSTGRES_PASSWORD}@postgres:5432/${POSTGRES_DB:-rk_reservations}
      ENVIRONMENT: production
    depends_on:
      postgres:
        condition: service_healthy
      ...
      - name: Deploy to EC2 via SSH
        ...
        with:
          ...
          script: |
            ...
            # Run DB migrations
            docker exec rk-reservations-api python -m db.migrate
...
# Clone repo
git clone https://github.com/rodinKaradeniz/rk-reservations.git ~/rk-reservations
```
...
CORS_ORIGINS=https://rk-reservations.vercel.app
STORAGE_TYPE=local
```
