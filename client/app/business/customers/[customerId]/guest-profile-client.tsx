...
function money(value?: number) {
  return value === undefined ? null : new Intl.NumberFormat(undefined, { style: "currency", currency: "USD" }).format(value);
}

export default function GuestProfileClient({ customerId, canManage }: { customerId: string; canManage: boolean }) {
  ...
  if (loading) return <div className="page-pad flex min-h-80 items-center justify-center text-sm text-muted-foreground"><Loader2 className="mr-2 size-4 animate-spin" /> Loading guest…</div>;
  if (!guest) return <div className="page-pad"><Link className="text-sm underline" href="/business/customers">Back to customers</Link><p className="mt-6 text-muted-foreground">Guest profile not found.</p></div>;

  return <div className="page-container">
    <div className="flex flex-wrap items-start justify-between gap-4">
      <div><Link href="/business/customers" className="inline-flex items-center text-sm text-muted-foreground hover:text-foreground"><ArrowLeft className="mr-1 size-4" /> Guests</Link><h1 className="page-title mt-3">{guest.name ?? "Guest"}</h1><p className="page-description">One business-scoped hospitality profile and operational history.</p></div>
      {canManage && <div className="flex gap-2"><Button variant="outline" size="sm" onClick={() => void downloadExport()} disabled={busy}><Download /> Export</Button><Button variant="destructive" size="sm" onClick={() => setDeleteOpen(true)} disabled={busy}><Trash2 /> Anonymise</Button></div>}
    </div>

    <div className="grid gap-6 xl:grid-cols-[minmax(0,1fr)_22rem]">
      <div className="space-y-6">
        <section className="rounded-xl border bg-card p-5"><div className="flex items-center justify-between gap-3"><h2 className="font-semibold">Guest details</h2>{guest.dietaryDetails && <Badge variant="destructive"><AlertTriangle className="mr-1 size-3" /> Dietary alert</Badge>}</div>
        ...
        <section className="rounded-xl border bg-card p-5"><h2 className="font-semibold">Guest timeline</h2><div className="mt-4 divide-y">{guest.timeline.length ? guest.timeline.map((entry) => <div className="flex items-start justify-between gap-4 py-3" key={entry.id}><div><p className="text-sm font-medium">{entry.title}</p>{entry.detail && <p className="mt-0.5 text-sm text-muted-foreground">{entry.detail}</p>}<p className="mt-1 text-xs text-muted-foreground">{format(parseISO(entry.occurredAt), "PPp")}{entry.status ? ` · ${entry.status}` : ""}</p></div>{money(entry.amount) && <span className="figures text-sm">{money(entry.amount)}</span>}</div>) : <p className="py-6 text-sm text-muted-foreground">No guest activity yet.</p>}</div></section>

        <section className="rounded-xl border bg-card p-5"><h2 className="font-semibold">Team notes</h2><form className="mt-4 space-y-3" onSubmit={addNote}><Input value={noteTitle} onChange={(event) => setNoteTitle(event.target.value)} placeholder="Note title" /><Textarea value={noteBody} onChange={(event) => setNoteBody(event.target.value)} placeholder="Add a useful service note" /><Button type="submit" size="sm" disabled={busy || !noteTitle.trim() || !noteBody.trim()}><Plus /> Add note</Button></form><div className="mt-5 space-y-3">{guest.notes.map((note) => <article className="rounded-lg bg-muted/40 p-3" key={note.id}><p className="text-sm font-medium">{note.title}</p><p className="mt-1 whitespace-pre-wrap text-sm">{note.body}</p><p className="mt-2 text-xs text-muted-foreground">Created {format(parseISO(note.createdAt), "PPp")} · last edited {format(parseISO(note.updatedAt), "PPp")}</p></article>)}</div></section>
      </div>

      <aside className="space-y-6">
        <section className="rounded-xl border bg-card p-5"><h2 className="flex items-center gap-2 font-semibold"><Tags className="size-4" /> Tags</h2><div className="mt-3 flex flex-wrap gap-2">{guest.tags.map((item) => <button key={item.id} type="button" className="rounded-full border px-2.5 py-1 text-xs hover:bg-muted" onClick={() => void clientRemoveGuestTag(customerId, item.id).then(refresh)}>{item.name} ×</button>)}</div><div className="mt-4 flex gap-2"><Input value={tag} onChange={(event) => setTag(event.target.value)} placeholder="Custom tag" /><Button size="icon" aria-label="Add tag" disabled={busy} onClick={() => void addTag()}><Plus /></Button></div><div className="mt-3 flex flex-wrap gap-2">{SUGGESTED_TAGS.filter((name) => !guest.tags.some((item) => item.name.toLowerCase() === name.toLowerCase())).map((name) => <Button key={name} size="sm" variant="outline" disabled={busy} onClick={() => void addTag(name)}>{name}</Button>)}</div></section>
        <section className="rounded-xl border bg-card p-5"><h2 className="font-semibold">Marketing consent</h2><div className="mt-3 space-y-3 text-sm">{guest.consents.length ? guest.consents.map((consent) => <div key={consent.channel}><p className="font-medium capitalize">{consent.channel}</p><p className="text-muted-foreground">{consent.isConsented ? "Opted in" : "Not opted in"} · {format(parseISO(consent.capturedAt), "PP")}</p></div>) : <p className="text-muted-foreground">No marketing consent recorded.</p>}</div></section>
        {canManage && <section className="rounded-xl border bg-card p-5"><div className="flex items-center justify-between gap-3"><h2 className="font-semibold">Reconcile duplicate</h2><Button size="sm" variant="outline" onClick={() => void loadMergeCandidates()} disabled={busy}><GitMerge /> Find guest</Button></div>{mergeCandidates.length > 0 && <div className="mt-3 space-y-2">{mergeCandidates.map((candidate) => <div key={candidate.id} className="flex items-center justify-between gap-2 rounded-lg bg-muted/40 p-2"><div className="min-w-0"><p className="truncate text-sm font-medium">{candidate.name ?? "Unnamed guest"}</p><p className="truncate text-xs text-muted-foreground">{candidate.phone ?? candidate.email ?? candidate.id}</p></div><Button size="sm" variant="outline" disabled={busy} onClick={() => void mergeGuest(candidate.id)}>Merge</Button></div>)}</div>}<p className="mt-3 text-xs text-muted-foreground">The selected duplicate is anonymised and its operational history moves here.</p></section>}
        <section className="rounded-xl border bg-muted/30 p-5 text-sm text-muted-foreground"><p className="font-medium text-foreground">Privacy</p><p className="mt-2">Profiles are anonymised after 24 months of inactivity, unless a venue has a lawful retention obligation. Marketing consent is separate from reservation messages.</p></section>
      </aside>
    </div>
