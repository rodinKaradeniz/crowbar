...
export default function OrderPage() {
  ...
  useEffect(() => {
    if (typeof window !== "undefined") {
      const stored = sessionStorage.getItem(`cart_${businessId}`);
      if (stored) {
        try {
  ...
  function removeItem(index: number) {
    ...
    setCart(next);
    if (typeof window !== "undefined") {
      sessionStorage.setItem(`cart_${businessId}`, JSON.stringify(next));
    }
  }
  ...
  async function placeOrder() {
    ...
    try {
      ...
      setCart([]);
      if (typeof window !== "undefined") {
        sessionStorage.removeItem(`cart_${businessId}`);
      }
    } catch (e) {
  ...
  if (sessionToken) {
    return (
      <div className="min-h-screen bg-background p-6 max-w-md mx-auto">
        ...
        <div className="mt-6">
          <Link href={`/menu/${businessId}${table ? `?table=${encodeURIComponent(table)}` : ""}`}>
