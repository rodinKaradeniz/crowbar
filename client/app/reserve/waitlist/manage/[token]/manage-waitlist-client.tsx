...
export default function ManageWaitlistClient() {
  ...
  return <main className="grid min-h-screen place-items-center p-6"><section className="w-full max-w-md rounded-xl border bg-card p-7 text-center">{active ? <CheckCircle2 className="mx-auto size-10 text-muted-foreground" /> : <XCircle className="mx-auto size-10 text-muted-foreground" />}<h1 className="mt-4 font-display text-3xl">Waitlist request</h1>{!entry && !error ? <p className="mt-3 text-sm text-muted-foreground">Loading request…</p> : error ? <><p className="mt-3 text-[length:var(--ui-size)] text-critical-text">{error}</p><Button className="mt-5" variant="secondary" onClick={() => void load()}>Try again</Button></> : <><p className="mt-3 text-sm text-muted-foreground">Status: {entry?.status.replaceAll("_", " ")}</p>{active && <Button className="mt-6" variant="secondary" disabled={busy} onClick={() => void cancel()}>{busy ? "Cancelling…" : "Cancel waitlist request"}</Button>}</>}</section></main>;
}
