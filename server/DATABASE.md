...
Integration tests use a separate `rk_reservations_test` database to avoid touching development data.

| Command | Description |
|---|---|
| `docker compose exec postgres createdb -U postgres rk_reservations_test` | Create the test database (one-time setup) |
| `docker compose exec postgres dropdb -U postgres rk_reservations_test` | Drop the test database |

The test suite automatically creates and drops all tables before/after each test — no manual migrations needed. The test database URL is derived from your main `DATABASE_URL` by replacing the database name with `rk_reservations_test` (see `tests/conftest.py`).
